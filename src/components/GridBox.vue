<template>
  <div :class="classObject" :style="{ backgroundColor: rgb }"></div>
</template>
...
const { explodingBoxesN, blinkingBoxesN, arrowIndex, gridArray, caughtBox } = storeToRefs(store)
...
const rgb = computed(() => {
  ...
  if (gridBoxValue === null || gridBoxValue === ARROW) {
    return ALICEBLUE
  } else {
